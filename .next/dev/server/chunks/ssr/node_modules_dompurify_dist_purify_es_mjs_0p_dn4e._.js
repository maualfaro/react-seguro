module.exports = [
"[project]/node_modules/dompurify/dist/purify.es.mjs [app-rsc] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
/*! @license DOMPurify 3.4.13 | (c) Cure53 and other contributors | Released under the Apache license 2.0 and Mozilla Public License 2.0 | github.com/cure53/DOMPurify/blob/3.4.13/LICENSE */ function _arrayLikeToArray(r, a) {
...
function createDOMPurify() {
   ...
   */ const _createNodeIterator = function _createNodeIterator(root) {
       ...
       returns the real Document regardless of the clobber. */ const doc = getOwnerDocument ? getOwnerDocument(root) : root.ownerDocument;
        return createNodeIterator.call(doc || root, root, // eslint-disable-next-line no-bitwise
   ...
   */ const _scrubTemplateExpressions2 = function _scrubTemplateExpressions(node) {
       ...
       carry a form-named-getter override of ownerDocument. */ const doc = getOwnerDocument ? getOwnerDocument(node) : node.ownerDocument;
        const walker = createNodeIterator.call(doc || node, node, // eslint-disable-next-line no-bitwise
   ...
   */ // eslint-disable-next-line complexity
    const _sanitizeElements = function _sanitizeElements(currentNode, root) {
       ...
       resource handler on it cannot fire in page scope after we return. */ if (currentNode !== root && getParentNode(currentNode) === null) {
            if (IN_PLACE) {
       ...
       reference. Fork it before exposing it to the hook. */ ALLOWED_TAGS = _forkSharedAllowlist(hooks.uponSanitizeElement, ALLOWED_TAGS, DEFAULT_ALLOWED_TAGS, SET_CONFIG_ALLOWED_TAGS);
       ...
       as the post-walk pass does for _forceRemove'd subtrees. */ if (currentNode !== root && getParentNode(currentNode) === null) {
            if (IN_PLACE) {
                _neutralizeSubtree(currentNode);
            }
            return true;
   ...
   */ const _sanitizeAttributes = function _sanitizeAttributes(currentNode) {
       ...
       attribute allowlist off its shared binding before a hook can see it. */ ALLOWED_ATTR = _forkSharedAllowlist(hooks.uponSanitizeAttribute, ALLOWED_ATTR, DEFAULT_ALLOWED_ATTR, SET_CONFIG_ALLOWED_ATTR);
        const hookEvent = {
            attrName: '',
            attrValue: '',
            keepAttr: true,
   ...
   */ const _sanitizeAttachedShadowRoots = function _sanitizeAttachedShadowRoots(root) {
     ...
     * from the captured bindings just above - so the two cannot diverge. */ if (hooks.uponSanitizeElement.length > 0 || hooks.uponSanitizeAttribute.length > 0) {
       ...
       body, so the propagating throw is already fail-closed there.) */ try {
...
//# sourceMappingURL=node_modules_dompurify_dist_purify_es_mjs_0p_dn4e._.js.map